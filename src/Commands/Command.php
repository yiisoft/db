...
{
    /**
     * @var Connection|null the DB connection that this command is associated with
     */
    ...
    /**
     * @var Dependency the dependency to be associated with the cached query result
     * for this command
     *
     * @see cache()
     */
    ...
    public function __construct(Profiler $profiler, LoggerInterface $logger, ?Connection $db, $sql)
    {
        $this->db = $db;
        $this->logger = $logger;
     ...
     * @param string $type the column type. {@see QueryBuilder::getColumnType()} will be called
    ...
    protected function queryInternal(string $method, $fetchMode = null)
    {
        ...
        try {
            $this->profiler->begin((string) $rawSql, ['\Yiisoft\Db\Command::query']);
            ...
            $this->profiler->end((string) $rawSql, ['\Yiisoft\Db\Command::query']);
        } catch (Exception $e) {
            $this->profiler->end((string) $rawSql, ['\Yiisoft\Db\Command::query']);
     ...
     * @param string $method method of PDOStatement to be called
     * @param int $fetchMode the result fetch mode.
     * Please refer to [PHP manual](https://secure.php.net/manual/en/function.PDOStatement-setFetchMode.php)
