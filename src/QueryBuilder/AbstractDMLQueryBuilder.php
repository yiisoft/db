...
abstract class AbstractDMLQueryBuilder implements DMLQueryBuilderInterface
{
    ...
    public function insertBatch(string $table, iterable $rows, array $columns = [], array $params = []): array
    {
        ...
        $parameters = $this->prepareBatchInsertValues($table, $rows, $columns);
     ...
     * @psalm-param ParamsType $params
     */
    protected function prepareBatchInsertValues(string $table, iterable $rows, array $columnNames, int $maxRowsAtOnce = 0): array
    {
        $parameters = [];
        /** @var string[] $names */
